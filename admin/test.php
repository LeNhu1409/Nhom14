...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        error_log("Đăng nhập thất bại: Thiếu thông tin đầu vào");
    } else {
        ...
        try {
            ...
            if ($admin) {
                if (password_verify($password, $admin['password'])) {
                    ...
                    error_log("Đăng nhập thất bại: Mật khẩu không đúng");
                }
            } else {
                $error = "Tài khoản không tồn tại!";
                error_log("Đăng nhập thất bại: Tài khoản không tồn tại");
            }
        } catch (PDOException $e) {
            $error = "Lỗi cơ sở dữ liệu: " . htmlspecialchars($e->getMessage());
...
<head>
    ...
    <style>
        /* Nền/ Nền ảnh động cho toàn trang */
        body {
            margin: 0;
            padding: 0;
